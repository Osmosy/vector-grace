...
- **[Неубиваемый пайплайн Claude→DeepSeek](references/claude-prototype-pipeline.md)** — крупная LLM (Claude, GPT-5, Gemini 2, DeepSeek V4 Pro) как прототипист (не архитектор): прототип кода с семантикой = few-shot для слабой LLM (DeepSeek, Grok, GLM). 2/3 токенов в дешёвую модель, 2x скорость. Проверен на сотнях проектов.
- **[Anthropization trap](references/anthropization-trap.md)** — антропизация пайплайна = главная ошибка. ТЗ для агентов — субоптимально, прототип с семантикой — нормально. Крупная LLM (Claude, GPT-5, Gemini 2, DeepSeek V4 Pro) тянет кривой пайплайн на мощи, деградация незаметна. Тест на средних LLM (DeepSeek, GLM) вскрывает кривизну.
