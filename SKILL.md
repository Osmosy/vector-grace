...
- **[Неубиваемый пайплайн Claude→DeepSeek](references/claude-prototype-pipeline.md)** — крупная LLM (любая frontier-модель) как прототипист (не архитектор): прототип кода с семантикой = few-shot для слабой LLM (DeepSeek, Grok, GLM). 2/3 токенов в дешёвую модель, 2x скорость. Проверен на сотнях проектов.
- **[Anthropization trap](references/anthropization-trap.md)** — антропизация пайплайна = главная ошибка. ТЗ для агентов — субоптимально, прототип с семантикой — нормально. Крупная LLM (любая frontier-модель) тянет кривой пайплайн на мощи, деградация незаметна. Тест на средних LLM (DeepSeek, GLM) вскрывает кривизну.
- **[Архитектура vs быстрый фикс](references/architecture-vs-quick-fix.md)** — архитектура-first правильна при известных требованиях и долгоживущей системе. Ловушка при неясных требованиях = overengineering. Баланс: достаточная архитектура + готовность к изменению. GRACE: синтез сверху вниз + пропорциональная гранулярность.
...
- **[«Всё — плагин»: DeepSeek Harness](references/plugin-everything-deepseek.md)** — эссе автора GRACE: DeepSeek подтвердил концепцию «всё является плагином» (модели, инструменты, навыки, сессии, песочницы, циклы, UI). Контекст: разочарование в микросервисах («распределённые монолиты»), ИИ нативно пишет 100–200 строк → плагин как единица разложения. Фундамент — Cordis (IoC, обратимые эффекты). Контрапункт: user-side cost конфигурации. Работает из РФ без VPN.
