...
## Обязательность семантической разметки

> Семантическая разметка — не опция, не рекомендация, не стилистическое предпочтение. Это обязательное требование GRACE-проекта. Без неё проект не является GRACE-проектом.

### Экономическое обоснование

Семантическая разметка — leverage, не overhead. Без неё ты платишь моделью:

- Средняя LLM (DeepSeek v4 Flash, GLM-5) с семантической разметкой в коде работает не хуже крупной (Claude Sonnet, GPT-4) без неё
- Крупная LLM с разметкой работает как команда — быстрее, точнее, меньше итераций
- Без разметки и средняя, и крупная LLM платят дополнительными итерациями, галлюцинациями и контекстом

Формула: разметка снижает неоднозначность → меньше нужно reasoning → слабее модель достаточна → дешевле исполнение.

Это не «Flash = Claude». Это «Flash + semantics = Claude − reasoning overhead». Для 80% задач (чтение, правка, поиск, генерация по паттерну) — достаточно. Оставшиеся 20% (архитектура, сложный дебаг, novel problems) — делегируй крупной модели точечно.

### Правила обязательности

1. **Ни одна строка production-кода без разметки.** Каждый файл начинается с MODULE_CONTRACT. Каждая функция с CONTRACT. Каждый блок ~500 токенов с START_BLOCK/END_BLOCK.

2. **Логи привязаны к блокам.** Формат `[Module][function][BLOCK_NAME]` — не стилистическое правило, а навигационный контракт. Агент грепает логи и находит код. Без привязки — агент слепой.

3. **Разметка — несущая конструкция.** Удалил — агент потерял навигацию. Редактируя код, сохрани блоки. Переименовал блок — обнови все log-ссылки и verification-записи.

4. **Проверка перед коммитом.** Если в файле нет MODULE_CONTRACT — файл не готов. Если в функции нет CONTRACT — функция не готова. Если в логах нет BLOCK_NAME — логи не готовы. `grace lint` должен это enforcement'ить.

5. **Пропорциональность.** Детализация разметки пропорциональна критичности компонента. Не размечай тривиальные геттеры. Размечай всё, что несёт бизнес-логику или нетривиальный поток данных.

6. **GREP-хинты обязательны для DeepSeek/GLM.** Если проект использует китайские модели — GREP-теги в коде не опциональны, а критичны для навигации. Агент грепает по умолчанию; не бороться, а возглавить.

### Что считать готовой разметкой

Чек-лист (controller проверяет перед принятием модуля):

- [ ] MODULE_CONTRACT с PURPOSE, SCOPE, DEPENDS, INPUTS, OUTPUTS
- [ ] MODULE_MAP с описанием экспортируемых/локальных символов
...
- [ ] Knowledge graph обновлён (public интерфейсы)
- [ ] Verification plan обновлён (маркеры, тесты)
- [ ] Для Python: Doxygen XML сгенерирован, `GENERATE_XML=YES`
